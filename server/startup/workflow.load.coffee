...
Meteor.startup ->
  ...
  @globals.BankForms = JSON.parse(Assets.getText('workflows/bankforms.json'))
  ...
  Accounts.validateNewUser (user) =>
    if Generics['ERPNextUsers'].find({"email": user.profile.email}).count() > 0 
      ...
      #Change this to return false for non ERP Next Users
      #For testing purposes this should be set to return true
      return true
      #return false
...
ServiceConfiguration.configurations.insert
    service: 'facebook'
    appId: '889365354572201'
    secret: 'e39d0275272bf8527cb5df23df346c4d'
