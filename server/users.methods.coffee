Meteor.methods
 addErpCookie: (targetUserEmail, cookie) ->
	...
	userExists: (userDetails) ->
		...
		emailAlreadyExist = Meteor.users.find({"emails.address": userDetails.email}, {limit: 1}).count()>0
	...
	getUserDetails: (userDetail) ->
	  ...
	  user = Meteor.users.findOne({"_id": userDetail.userId})
	  return user
    
    
  updateUserProfile: (userDetails) ->
      Meteor.users.update({_id: userDetails.userId}, {$set: {"profile.name": userDetails.name, "profile.phone": userDetails.phone, "profile.dob": userDetails.dob}})
