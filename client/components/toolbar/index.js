	...
	componentWillReceiveProps(nextProps) {
		...
		if (nextProps.dashboardName) {
			...
			if (nextProps.dashboards != {}) {
				...
				this.setState({ currentDashboard: nextProps.dashboards[nextProps.dashboardName] })
		...
		var managerList = managers.map((manager, index) => (
			...
			<div className="group-user" key={index}>
				...
				<div>
					...
					<img src={manager.profile.roles ? 'assets/checkmark.png' : 'assets/questionmark.png'} width={30} height={30} />
									...
									<ul className="nav navbar-nav navbar-right">
													...
													<a href className="dropdown-toggle" data-toggle="dropdown" role="button">
														<span className="glyphicon glyphicon-stats"></span> {this.state.currentDashboard ? this.state.currentDashboard.label : null}
														{
															(this.state.currentDashboard && this.state.currentDashboard.notifications) ?
																<sup className="notification">
																	{this.state.currentDashboard.notifications / 2}
																</sup> : null
														}
										...
										<li className="dropdown">
											<a href className="dropdown-toggle" data-toggle="dropdown" role="button">
												...
												{(this.props.currentUser && this.props.currentUser.profile) ? this.props.currentUser.profile.name : null}
												<span className="caret"></span>
											</a>
															...
															<tr>
																<td>
																	...
																	<span>
																		...
																		<small>
																			{(this.props.currentUser && this.props.currentUser.profile && this.props.currentUser.profile.groups) ? this.props.currentUser.profile.groups[this.props.currentUser.profile.groups.length - 1] : "Guest"}
																		</small>
																	</span>
																</td>
																<td>
																	...
																	<a className="btn btn-primary manage-button" role="menuitem" onClick={() => this.goToUserBookMarks()}>
																		<span className="glyphicon glyphicon-bookmark"></span> &nbsp; Bookmarks
																	</a>
																	<a className="btn btn-primary manage-button" role="menuitem" onClick={() => this.goToUserForms()}>
																		&nbsp; Forms
																	</a>
																	<a className="btn btn-primary manage-button" role="menuitem" onClick={() => this.goToUserProfile()}>
